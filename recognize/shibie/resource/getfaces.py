...
path_photos_from_camera = "data/load/data_faces_from_camera/"

def intlx():
    # Dlib 正向人脸检测器 / frontal face detector
    detector = dlib.get_frontal_face_detector()
    # Dlib 68 点特征预测器 / 68 points features predictor
    #predictor = dlib.shape_predictor('data/data_dlib/shape_predictor_68_face_landmarks.dat')-----------------------------2019.10.3
    ...
    def pre_work_del_old_face_folders():
        ...
        if os.path.isfile("data/load/features_all.csv"):
            os.remove("data/load/features_all.csv")
    ...
    if os.listdir("data/load/data_faces_from_camera/"):
    ...
    while cap.isOpened():
        ...
        #img_gray = skimage.exposure.adjust_gamma(img_gray, gamma=0.2, gain=1)
        ...
        if len(faces) != 0:
            ...
            for k, d in enumerate(faces):
                ...
                if ((d.right()-d.left()) < 80) or ((d.bottom()-d.top())<80):
                    cv2.putText(img_rd, "please close to camera", (300, 300), font, 0.8, (0, 0, 255), 1, cv2.LINE_AA)
                    color_rectangle = (0, 0, 255)#red
                    save_flag = 0
                    if kk & 0xFF == ord('s'):
                        cv2.putText(img_rd, "please adjust your position!", (10, 100), font, 1.3, (0, 0, 255), 2,
                                    cv2.LINE_AA)
                # 判断人脸矩形框是否超出 480x640
                elif (d.right())+ww > 640 or (d.bottom()+hh>480) or (d.left()-ww < 0) or (d.top()-hh < 0):
                    cv2.putText(img_rd, "OUT OF RANGE", (20, 300), font, 0.8, (0, 0, 255), 1, cv2.LINE_AA)
                    color_rectangle = (0, 0, 255)#red
                    save_flag = 0
                    if kk & 0xFF == ord('s'):
                        cv2.putText(img_rd, "please adjust your position!", (10, 100), font, 1.3, (0, 0,255), 2, cv2.LINE_AA)
                ...
                im_blank = np.zeros((hh*5, ww*6, 3), np.uint8)
                ...
                if save_flag:
                    ...
                    if kk & 0xFF == ord('s'):
                        ...
                        if press_n_flag:
                            cnt_ss += 1
                            for ii in range(hh*5):
                                for jj in range(ww*6):
        ...
        cv2.putText(img_rd, "new person click:N", (20, 350), font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
        cv2.putText(img_rd, "save face click:S", (20, 400), font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
        cv2.putText(img_rd, "get images already click:Q", (20, 450), font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
